...
def main():
    ...
    y = labels
    print(f"Selected {len(selected_texts)} sentences (label distribution: {np.bincount(y)})")

    # 3. Extract CLS embeddings from BERT-base
    print("Extracting CLS embeddings from bert-base-uncased ...")
    ...
    # 4. Baseline probe on original embeddings
    print("\n=== Gender probe on ORIGINAL embeddings ===")
    metrics_orig = train_and_eval_probe(X, y)
    print(f"Accuracy: {metrics_orig['accuracy']:.4f}, F1-weighted: {metrics_orig['f1_weighted']:.4f}")

    # 5. Learn INLP projection (or reuse if you prefer)
    print("\nLearning INLP projection ...")
    ...
    # 6. Apply projection and re-evaluate probe
    print("\nApplying INLP projection and re-evaluating probe ...")
    ...
    out_path = out_dir / "inlp_gender_probe_results.json"
